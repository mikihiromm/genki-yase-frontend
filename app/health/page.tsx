...
export default function HealthPage() {
  ...
  const save = async () => {
    ...
    const res = await fetch(`${API}/api/health`, {
      ...
      body: JSON.stringify({ userId: USER_ID, weight: weight ? parseFloat(weight) : null, systolic: systolic ? parseInt(systolic) : null, diastolic: diastolic ? parseInt(diastolic) : null, recordedAt: new Date().toISOString() }),
    });
    if (res.ok) setSaved(true);
    else setError('記録に失敗しました。もう一度お試しください。');
  };
